...
test "Quickstart example" {
  ...
  let rules = [
    @lib.Rule::{
      name: "High Workload",
      metric: @lib.Metric::AcuteLoad,
      op: @lib.Operator::GreaterThan,
      threshold: 300.0,
      window_size: 1,
      severity: @lib.RiskLevel::Medium,
      description: "Workload exceeds 300."
    }
  ]
  
  // 3. Assess injury risk
  let assessment = @lib.assess_injury_risk(records, "2026-08-01", rules)
  
  // 4. Verify assessment
  assert_true(assessment is Some(_))
  let report = assessment.unwrap()
  assert_true(report.risk_level is @lib.RiskLevel::Medium)
}
```
